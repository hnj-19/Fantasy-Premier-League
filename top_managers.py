...
def top_managers(leagueID=314, season= "2021-22"):
	if leagueID == 314:
		overallLeagueID = "overall"
	else:
		overallLeagueID = leagueID
	...
	if season == "2019-20":
		...
		#gameweeks for normal seasons
		gameWeeks = list(range(1,38))
	...
	manager_data = open(rf'data/{season}/managers/top_managers_{str(overallLeagueID)}.csv', 'w', newline='', encoding="utf-8")
	gw_data = open(rf'data/{season}/managers/top_managers_{str(overallLeagueID)}_gwInfo.csv', 'w', newline='', encoding="utf-8")
	gw_picks = open(rf'data/{season}/managers/top_managers_{str(overallLeagueID)}_gwPicks.csv', 'w', newline='', encoding="utf-8")
	...
	df = pd.read_csv(rf'data/{season}/managers/top_managers_{str(overallLeagueID)}_gwPicks.csv')
	df1 = pd.read_csv(rf'data/{season}/player_idlist.csv') 
	...
	merged.to_csv(rf'data/{season}/managers/top_managers_{str(overallLeagueID)}_gwPicks.csv',index=False)

def main():
    top_managers(int(sys.argv[1]), sys.argv[2])
